...
├── scripts/
│   ├── seedSyntheticUsers.ts    # Synthetic listener profiles seeder script
│   └── test-rag.js              # Standalone RAG engine verification script
├── src/
│   ├── app/                     # Next.js 16 App Router pages & API webhooks
│   │   ├── api/
│   │   │   ├── analysis/        # OCEAN & AI narrative API endpoints
│   │   │   ├── auth/            # NextAuth.js Spotify OAuth callbacks
│   │   │   ├── jam-rooms/       # Dynamic Jam Rooms API endpoints
│   │   │   ├── spotify/         # Cached Spotify Web API proxies
│   │   │   └── telegram/        # Live Telegram Bot webhook route
│   │   ├── dashboard/           # Liquid Glass Dashboard sub-routes
│   │   │   ├── jam-partners/    # Multi-vector candidate matching UI
│   │   │   ├── jam-rooms/       # Real-time Jam Room chat & player UI
│   │   │   ├── model-diff/      # ML model semver diff inspector
│   │   │   ├── personality-profile/ # Detailed Big Five breakdown
│   │   │   ├── quiz/            # 10-Item Mini-IPIP Ground-Truth survey
│   │   │   └── upload-history/  # Client-side streaming zip uploader
│   │   ├── globals.css          # Liquid Glass Design System CSS tokens
│   │   └── page.tsx             # Landing hero & feature showcase page
│   ├── components/              # Modular UI components & radar charts
│   ├── config/                  # OCEAN scoring weights & prompt configurations
│   ├── data/                    # Synthetic user profiles JSON dataset
│   ├── lib/                     # Core algorithmic engines & test suites
│   │   ├── auth.ts              # NextAuth options & token refresh rotation
│   │   ├── dynamicRoomEngine.ts # Dynamic room generation & genre clustering
│   │   ├── extendedHistory.ts   # In-browser ZIP streaming history parser
│   │   ├── features.ts          # Calibrated Shannon entropy & circadian signals
│   │   ├── feedbackStore.ts     # User trait feedback correction pairs
│   │   ├── genreClusters.ts     # Rentfrow & Gosling MUSIC cluster classifier
│   │   ├── ipipQuiz.ts          # Mini-IPIP ground truth & Pearson r math
│   │   ├── jamMatching.ts       # Psychometric candidate matching engine
│   │   ├── moodRoomEngine.ts    # Curated room definitions & vibe scoring
│   │   ├── narrativePrompt.ts   # OpenRouter AI narrative prompt builder
│   │   ├── oceanScoring.ts      # Big Five (OCEAN) scoring & attribution
│   │   ├── redis.ts             # Upstash Redis serverless cache helper
│   │   ├── ridgeRegression.ts   # L2 Ridge Regression solver & semver bumper
│   │   ├── roomPlaylistSource.ts # Agentic RAG Ochiai vector space sourcing
│   │   ├── spotify.ts           # Spotify API fetcher & rate-limit handler
│   │   └── syntheticUsers.ts    # Synthetic profile loading & filtering
│   └── types/                   # TypeScript interfaces & NextAuth declarations
